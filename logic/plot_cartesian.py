...
def plot_cartesian_graph(state):
    tree = state['tree']
    geom_values, delta_exp_values, scaled_x = [], [], []
    import numpy as np
    for item in tree.get_children():
        ...
        if dv_raw != "":
            dv = float(dv_raw)
            geom_values.append(gv); delta_exp_values.append(dv)
            scaled_x.append((gv*1e4)/(12*np.pi))
    fig = state['cartesian_figure']; canvas = state['cartesian_canvas']
    fig.clear(); ax = fig.add_subplot(111)
    if geom_values and delta_exp_values:
        ax.scatter(geom_values, delta_exp_values, color='blue', marker='o', label='δ vs G_i')
        slope, intercept, r_value, p_value, std_err = stats.linregress(geom_values, delta_exp_values)
        ...
        ax.scatter(scaled_x, delta_exp_values, color='red', marker='o', label='δ vs Scaled G_i')
        slope_r, inter_r, r_r, p_r, se_r = stats.linregress(scaled_x, delta_exp_values)
